...
#### Annotating PVCs for Backup

Add the `pbs-plus.io/backup` annotation to any PVC you want to back up:

```bash
kubectl annotate pvc my-pvc pbs-plus.io/backup=true
```

The operator will automatically create a backup agent pod that mounts the PVC and connects to the PBS Plus server.

#### Snapshot Support for RWO Volumes

For `ReadWriteOnce` (RWO) volumes, the operator automatically creates a VolumeSnapshot, restores it to a temporary PVC, and mounts that PVC to the backup agent. This allows backing up RWO volumes without disrupting workloads.

For `ReadWriteMany` (RWX) volumes, you can optionally enable snapshot mode:
...
- The server listens on port `8008` for aRPC (Agent RPC) using both:
  - `UDP/8008` (QUIC) — persistent control plane: ping, status, file-tree browsing, backup/restore initiation. Uses mTLS with QUIC.
  - `TCP/8008` (mTLS) — on-demand data plane: backup/restore binary data streams over smux. Only established during active backup/restore jobs.
