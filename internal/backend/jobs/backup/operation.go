...
func (b *BackupOperation) waitForCompletion(cmd *exec.Cmd, task proxmox.Task, currOwner string, agentMount *mount.AgentMount, s3Mount *mount.S3Mount) {
	...
	if err := updateBackupStatus(false, 0, job, task, b.storeInstance); err != nil {
		if currOwner != "" {
			_ = SetDatastoreOwner(job, b.storeInstance, currOwner)
		}
	}
	...
	if err := updateBackupStatus(succeeded, warningsNum, currentJob, task, b.storeInstance); err != nil {
