...
func TestOperationContractsValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "host agent restore request", value: HostAgentRestoreRequest{Operation: operation, Hostname: "agent", VolumeID: "disk", DestinationPath: "/restore"}},
...
func TestOperationContractsRejectInvalidValues(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "agent restore missing hostname", value: HostAgentRestoreRequest{Operation: operation, VolumeID: "disk", DestinationPath: "/restore"}, wantError: "agent hostname is required"},
