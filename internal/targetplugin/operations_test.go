...
func TestOperationContractsValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "restore open response", value: RestoreOpenResponse{Path: "/restore", CleanupToken: []byte("lease")}},
...
func TestOperationContractsRejectInvalidValues(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing cleanup token", value: RestoreOpenResponse{Path: "/restore"}, wantError: "cleanup token is required"},
		{name: "oversized cleanup token", value: RestoreOpenResponse{Path: "/restore", CleanupToken: make([]byte, maxLeaseTokenBytes+1)}, wantError: "cleanup token exceeds"},
