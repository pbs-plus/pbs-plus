...
const (
	...
	clientFamilyField = "client_family"
...
func Descriptor() targetplugin.Descriptor {
	...
	return targetplugin.Descriptor{
		...
		TargetSchema: targetplugin.FormSchema{Version: targetSchemaVersion, Fields: []targetplugin.FormField{
			...
			{Key: variantField, Label: "Server Family", Control: targetplugin.ControlSelect, Options: []targetplugin.SelectOption{
				{Label: "MySQL", Value: targetplugin.NewStringScalar("mysql")},
				{Label: "MariaDB", Value: targetplugin.NewStringScalar("mariadb")},
			}},
			{Key: tlsModeField, Label: "TLS Mode", Control: targetplugin.ControlSelect, Options: []targetplugin.SelectOption{
				...
				{Label: "Preferred", Value: targetplugin.NewStringScalar("preferred")},
				{Label: "Required", Value: targetplugin.NewStringScalar("required")},
				{Label: "Verify CA", Value: targetplugin.NewStringScalar("verify-ca")},
				{Label: "Verify CA and hostname", Value: targetplugin.NewStringScalar("verify-identity")},
			}},
			{Key: caCertField, Label: "CA Certificate", Control: targetplugin.ControlCertificatePath},
			{Key: clientFamilyField, Label: "Client Family", Control: targetplugin.ControlSelect, Options: []targetplugin.SelectOption{
				{Label: "MySQL", Value: targetplugin.NewStringScalar("mysql")},
				{Label: "MariaDB", Value: targetplugin.NewStringScalar("mariadb")},
			}},
...
func databaseTarget(job targetplugin.JobInput) (coredb.Target, string, error) {
	...
	variant, _ := config[variantField].StringValue()
	tlsMode, _ := config[tlsModeField].StringValue()
	caCertificate, _ := config[caCertField].StringValue()
	clientFamily, _ := config[clientFamilyField].StringValue()
	...
	return coredb.Target{
		...
		DatabaseClientFamily:     clientFamily,
...
func normalizeConfig(config targetplugin.Values) (targetplugin.Values, error) {
	...
	if variant != "mysql" && variant != "mariadb" {
		return nil, fmt.Errorf("unsupported MySQL variant %q", variant)
	}
	normalized[variantField] = targetplugin.NewStringScalar(variant)
	tlsMode, _ := config[tlsModeField].StringValue()
	if tlsMode == "" {
		tlsMode = "preferred"
	}
	...
	default:
		return nil, fmt.Errorf("unsupported MySQL TLS mode %q", tlsMode)
	}
	normalized[tlsModeField] = targetplugin.NewStringScalar(tlsMode)
	clientFamily, _ := config[clientFamilyField].StringValue()
	if clientFamily == "" {
		clientFamily = variant
	}
	if clientFamily != "mysql" && clientFamily != "mariadb" {
		return nil, fmt.Errorf("unsupported MySQL client family %q", clientFamily)
	}
	normalized[clientFamilyField] = targetplugin.NewStringScalar(clientFamily)
