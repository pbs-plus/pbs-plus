...
func (ev *CalendarEvent) String() string {
	var parts []string

	if ev.Weekdays != nil {
		wdNames := make([]string, len(ev.Weekdays))
		for i, wd := range ev.Weekdays {
			wdNames[i] = strings.ToLower(wd.String()[:3])
		}
		parts = append(parts, strings.Join(wdNames, ","))
	}

	hasDate := ev.Years != nil || ev.Months != nil || ev.Days != nil
	if hasDate {
		yearStr := renderField(ev.Years)
		monthStr := renderField(ev.Months)
		dayStr := renderField(ev.Days)
		if ev.Years != nil {
			parts = append(parts, fmt.Sprintf("%s-%s-%s", yearStr, monthStr, dayStr))
		} else {
			parts = append(parts, fmt.Sprintf("%s-%s", monthStr, dayStr))
		}
	}

	hasTime := ev.Hours != nil || ev.Minutes != nil || ev.Seconds != nil
	if hasTime {
		hourStr := renderField(ev.Hours)
		minStr := renderField(ev.Minutes)
		secStr := renderField(ev.Seconds)
		if ev.Seconds != nil && !(len(ev.Seconds) == 1 && ev.Seconds[0].Value == 0 && ev.Seconds[0].Repeat == 0) {
			parts = append(parts, fmt.Sprintf("%s:%s:%s", hourStr, minStr, secStr))
		} else {
			parts = append(parts, fmt.Sprintf("%s:%s", hourStr, minStr))
		}
	}
...
func renderField(specs []DateTimeValue) string {
	if specs == nil {
		return "*"
	}
	...
	for i, s := range specs {
		if s.Repeat > 0 {
			strs[i] = fmt.Sprintf("%d/%d", s.Value, s.Repeat)
		} else {
			strs[i] = fmt.Sprintf("%d", s.Value)
		}
	}
