...
var backupPanel = js.Panel{
	...
	Controller: js.Controller{Methods: map[string]js.Raw{
		"onSearchKeyUp":      searchFilter("id", "target", "ns", "comment", "subpath"),
		"addJob":             openEditWindow("PBS.D2DManagement.BackupJobEdit", ""),
		"editJob":            editJobWindow("PBS.D2DManagement.BackupJobEdit"),
		"duplicateJob":       duplicateJobWindow("PBS.D2DManagement.BackupJobEdit"),
		"removeJobs":         confirmRemove("/api2/extjs/config/disk-backup/", "encodePathValue(rec.getId())", "Remove selected entries?"),
		"openTaskLog":        openTaskLog("last-run-upid"),
		"openSuccessTaskLog": openTaskLog("last-successful-upid"),
		"runJobs":            runJobs(backupRunURL, "Start backup job '{0}'?", "Start backup jobs '{0}'?"),
	...
	Tbar: []js.Tool{
		...
		{Text: "Show job history", Handler: "showJobHistory", Disabled: true, EnableFn: enableOnSingleSelection}, js.Sep(),
		{Text: "Run Job(s)", Handler: "runJobs", Disabled: true, EnableFn: selectionEvery(jobIdle)},
		{Text: "Stop Job(s)", Handler: "stopJobs", Disabled: true, EnableFn: selectionEvery(jobStoppable)}, js.Sep(),
