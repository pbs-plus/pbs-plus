...
var selectors = []js.Value{
	...
	js.Selector{
		...
		ListColumns: []js.Column{
			...
			{Text: "Type", DataIndex: "target_type", Sortable: new(true), Flex: 1, Renderer: js.Func("value", `let icons = { local: '<i class="fa fa-desktop"></i> Local', agent: '<i class="fa fa-server"></i> Agent', s3: '<i class="fa fa-cloud"></i> S3', postgresql: '<i class="fa fa-database"></i> PostgreSQL', mysql: '<i class="fa fa-database"></i> MySQL / MariaDB', ldap: '<i class="fa fa-sitemap"></i> LDAP / AD' }; return icons[value] || Ext.String.htmlEncode(value || "");`)},
			{Text: "Location", DataIndex: "path", Sortable: new(true), Flex: 3, Renderer: js.Func("value, metaData, record", `if (record.get("target_type") === "agent") { return Ext.String.htmlEncode(record.get("volume_name") || record.get("volume_id") || "-"); } if (["postgresql", "mysql", "ldap"].includes(record.get("kind"))) { return Ext.String.htmlEncode(record.get("database_host") + ":" + record.get("database_port")); } return value ? Ext.String.htmlEncode(value) : "-";`)},
		},
		Methods: map[string]js.Raw{"initComponent": js.ChangerExtraParams},
