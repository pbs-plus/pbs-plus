...
func NewServer(storeInstance *store.Store, version string) (*Server, error) {
	...
	apiMux.HandleFunc("/api2/extjs/config/mtf-changer/{name}", ServerOnly(storeInstance, api.ExtJsMtfChangerSingleHandler(storeInstance)))
	apiMux.HandleFunc("/api2/extjs/config/mtf-drive", ServerOnly(storeInstance, api.ExtJsMtfDriveHandler(storeInstance)))
	apiMux.HandleFunc("/api2/extjs/config/mtf-drive/{name}", ServerOnly(storeInstance, api.ExtJsMtfDriveSingleHandler(storeInstance)))
