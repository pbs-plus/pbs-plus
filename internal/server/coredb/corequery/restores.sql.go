...
const deleteRestoreDovecotOptions = `-- name: DeleteRestoreDovecotOptions :exec
DELETE FROM restore_dovecot_options WHERE restore_id = ?
`

func (q *Queries) DeleteRestoreDovecotOptions(ctx context.Context, restoreID string) error {
	_, err := q.db.ExecContext(ctx, deleteRestoreDovecotOptions, restoreID)
	return err
}
...
type GetRestoreRow struct {
	...
	DovecotSourceUsername       string         `json:"dovecot_source_username"`
	DovecotDestinationUsername  string         `json:"dovecot_destination_username"`
	DovecotMailbox              string         `json:"dovecot_mailbox"`
	DovecotReplaceExisting      int64          `json:"dovecot_replace_existing"`
...
func (q *Queries) GetRestore(ctx context.Context, id string) (GetRestoreRow, error) {
	...
	err := row.Scan(
		...
		&i.DatabaseClientDir,
		&i.DovecotSourceUsername,
		&i.DovecotDestinationUsername,
		&i.DovecotMailbox,
		&i.DovecotReplaceExisting,
		&i.Name,
		&i.TargetType,
		&i.MountScript,
		&i.FilesystemAccess,
...
const upsertRestoreDovecotOptions = `-- name: UpsertRestoreDovecotOptions :exec
...
type UpsertRestoreDovecotOptionsParams struct {
	RestoreID           string `json:"restore_id"`
...
func (q *Queries) UpsertRestoreDovecotOptions(ctx context.Context, arg UpsertRestoreDovecotOptionsParams) error {
	_, err := q.db.ExecContext(ctx, upsertRestoreDovecotOptions,
		arg.RestoreID,
		arg.SourceUsername,
		arg.DestinationUsername,
		arg.Mailbox,
		arg.ReplaceExisting,
	)
	return err
