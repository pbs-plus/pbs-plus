...
func (db *Store) CreateBackup(tx *Transaction, backup Backup) (err error) {
	...
	err = q.CreateBackup(db.ctx, corequery.CreateBackupParams{
		...
		Expand7z:           backup.ExpandSevenZip,
		ExpandMaxDepth:     int64(backup.ExpandMaxDepth),
...
func (db *Store) GetBackup(id string) (Backup, error) {
	...
	backup := Backup{
		...
		ExpandZip:        row.ExpandZip,
		ExpandSevenZip:   row.Expand7z,
		ExpandMaxDepth:   int(row.ExpandMaxDepth),
