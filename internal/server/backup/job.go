...
func (b *backupJob) startBackup(ctx context.Context, srcPath string, info jobs.ActivityInfo) (*exec.Cmd, proxmox.Task, string, error) {
	select {
	case <-ctx.Done():
	...
	cmd, err := prepareBackupCommand(ctx, job, b.app, srcPath, extraExclusions, pluginLease, b.logger)
	if err != nil {
