...
The server uses SQLite (via `modernc.org/sqlite` - no CGo) for job, target, token, exclusion, and script storage. Migrations live in `internal/server/coredb/migrations/`. Query code is generated by sqlc (`sqlc.yaml`).
...
The internal contract is `kind: filesystem|s3` plus a kind-specific detail row. The target API also returns `kind` and `access`, while keeping the legacy `target_type: local|agent|s3` value for existing clients.
